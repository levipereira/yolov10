...
class Model(nn.Module):
    ...
    def quantize(
        ...
        **kwargs,
        ...
        This method facilitates model training with a range of customizable settings and configurations. It supports
        training with a custom quantizer or the default training approach defined in the method. The method handles
        different scenarios, such as resuming training from a checkpoint, integrating with Ultralytics HUB, and
        updating model and configuration after training.
        ...
        Args:
            quantizer (BaseTrainer, optional): An instance of a custom quantizer class for training the model. If None, the
                method uses a default quantizer. Defaults to None.
            **kwargs (any): Arbitrary keyword arguments representing the training configuration. These arguments are
                used to customize various aspects of the training process.
        ...
        overrides = yaml_load(checks.check_yaml(kwargs["cfg"])) if kwargs.get("cfg") else self.overrides
        custom = {"data": DEFAULT_CFG_DICT["data"] or TASK2DATA[self.task]}  # method defaults
        args = {**overrides, **custom, **kwargs, "mode": "quantize"}  # highest priority args on the right
        if args.get("resume"):
            args["resume"] = self.ckpt_path

        self.quantizer = (quantizer or self._smart_load("quantizer"))(overrides=args, _callbacks=self.callbacks)
        if not args.get("resume"):  # manually set model only if not resuming
            self.quantizer.model = self.quantizer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
        ...
        self.quantizer.hub_session = self.session  # attach optional HUB session
        self.quantizer.quantize()
        # Update model and cfg after training
        if RANK in (-1, 0):
            ckpt = self.quantizer.best if self.quantizer.best.exists() else self.quantizer.last
            self.model, _ = attempt_load_one_weight(ckpt)
            self.overrides = self.model.args
            self.metrics = getattr(self.quantizer.validator, "metrics", None)  # TODO: no metrics returned by DDP
        return self.metrics 
